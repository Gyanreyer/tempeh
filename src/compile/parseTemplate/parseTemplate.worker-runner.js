 ...
 */
const workerPool = [];
const workerPoolEventTarget = new EventEmitter();
workerPoolEventTarget.setMaxListeners(0);
...
const getFutureWorkerPromiseCB = (resolve) => {
  ...
  workerPoolEventTarget.once(`push:${awaitingWorkerID}`, function onPush() {
...
const returnWorkerToPool = (worker) => {
  ...
  workerPoolEventTarget.emit(`push:${awaitingWorkerCount--}`);
};
